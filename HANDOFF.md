...
### cron — 팩마다 독립 트리거, 현재 **켜져 있음** (5분 간격)
```jsonc
// wrangler.jsonc
"crons": [
  "*/5 * * * *",       // kr
...
**Workers Paid로 업그레이드해서 간격을 7분 → 5분으로 당김** (하루 4팩 합쳐 ~4,700회 정도,
월 100만 건 포함량의 15% 안팎이라 여유 있음). 오픈 전 한도 절약 목적으로 한동안
`crons: []`(완전 꺼둠)였는데, Paid 업그레이드 이후 이 세션에서 다시 켜서 배포 완료함
(commit `15a254a`, 배포 버전 `d2fd03de`). **다음 세션에서 이 상태를 신뢰해도 됨** — 껐다 켰다
하는 히스토리가 있으니 실제로 갱신되고 있는지 헷갈리면 `/debug?pack=kr` 응답의 `at` 타임스탬프로
최근 5분 이내인지 확인.
...
- **kr (7개 언론사, 53개 피드)**: 연합뉴스, SBS, 조선일보, 동아일보, 매일경제, 한국경제, 머니투데이
  ...
  - **⚠️ 미해결 이슈 (이번 세션에도 재확인됨)**: **한국경제(hankyung.com) 9개 피드 전부 여전히 403**. `/debug?pack=kr` 응답에 매번 뜸. 위 두 언론사랑 같은 패턴(Workers에서만 차단)일 가능성이 있어서 다음 세션에 확인 필요 — 아직 손 안 댐
- **us (12개 언론사)**: NYT, CNN, NPR, ABC News, CBS News, Fox News, BBC, CNBC, MarketWatch, Yahoo Finance, Forbes, Investing.com
  - ⚠️ 기존 CNN `edition.rss` 피드 하나가 계속 실패 중 — 예전엔 "Network connection lost"였는데 이번 세션에 `/debug?pack=us`로 다시 보니 **HTTP 525**(SSL handshake failed, Cloudflare↔오리진 사이 TLS 문제)로 바뀌어 있음. 새로 추가한 CNN world/tech 피드는 정상이라 이 특정 URL만의 문제로 보임, 여전히 미해결
...
**이번 세션 (2026-07-16, 노트북 → 이제 집으로 이동)**
1. **news-ticker**: 속보(breaking) 표시 배지를 `.ticker-article`(제목 옆)에서 `.ticker-source`
   왼쪽 위 모서리로 이동, 절대 위치 배지로 크기도 6px → 4px 축소해서 덜 거슬리게 함
   (`content.js`, `ticker.css`, commit `678795b`, push 완료)
2. **news-ticker-worker**: `cleanText()` HTML 엔티티 디코딩 개선 — 숫자 엔티티(10진 `&#39;`/16진
   `&#x27;` 등) 범용 처리 추가 + 이름 엔티티(`lt/gt/quot/apos/nbsp`) 확장. 머니투데이 등 일부
   언론사가 `&#039;`처럼 0-padded 숫자 엔티티를 써서 기존 정규식(`&#39;`만 매칭)이 놓치던 케이스
   해결. 공백도 정규화(`\s+` → 단일 스페이스)
3. **Workers Paid로 업그레이드** → cron을 다시 켜고 간격도 7분 → 5분으로 당김 (오픈 전
   한도 절약 목적으로 꺼뒀던 `crons: []` 상태를 해제). `src/index.js`의 `CRON_PACK` 매핑도
   5분 기준 cron 문자열로 같이 수정 (commit `15a254a`, push + `npm run deploy` 배포 완료,
   버전 `d2fd03de-4949-4e55-90bd-251716c3661f`)
4. 기존 미해결 이슈 재확인: 한국경제(hankyung.com) 9개 피드 여전히 403 / CNN `edition.rss`는
   이번엔 HTTP 525로 실패 (증상이 바뀜, 근본 원인은 아직 미조사) — 둘 다 손대지 않고 확인만 함
5. 양쪽 저장소 전부 커밋 + push 완료, 워커는 배포까지 완료된 상태로 세션 마무리
...
7. kr 한국경제 403, us CNN edition.rss 연결 문제 발견 (둘 다 미해결, 이후 세션들에서도 계속 재확인만 하고 미착수)
8. **(다른 노트북에서)** 새 맥 개발 환경 처음부터 셋업 완료: Xcode 라이선스, GitHub SSH 키, nvm+Node.js v24, `npm install`, `wrangler login`
9. `news-ticker-worker`의 cron을 한동안 완전히 꺼뒀었음 (`wrangler.jsonc`, commit `bcd7777`, 오픈 전 Cloudflare 한도 절약 목적) — **이번 세션에 Workers Paid 업그레이드하면서 다시 켜서 지금은 5분 간격으로 정상 동작 중** (위 3번 항목 참고)

**훨씬 이전 세션**
- 워커에 노출 순서 그룹핑 로직 최초 추가 (이후 클라이언트로 이동함)
...
- **미해결 이슈 확인** (우선순위 높음, 여러 세션째 계속 미룸): kr 한국경제(hankyung.com) 9개 피드 전부 403 / us CNN `edition.rss`가 HTTP 525로 실패 — 둘 다 `/debug?pack=kr`, `/debug?pack=us`로 재확인 가능. 서울경제/헤럴드경제/연합뉴스TV랑 같은 "Workers에서만 차단" 패턴인지 로컬 curl이랑 비교해보면 좋음
- **cron은 이제 켜져 있고 배포도 끝난 상태** — 5분 간격으로 정상 갱신 중 (위 4번 섹션 참고). 더 이상 신경 쓸 필요 없음, 확인만 하고 싶으면 `/debug?pack=kr`의 `at` 타임스탬프가 최근인지 보면 됨
- **브랜딩/디자인 수정** — 계속 대기 중 (여러 세션 전부터 예정돼 있었는데 매번 다른 작업이 먼저 들어옴)
...
- 이번 세션에서 고친 엔티티 디코딩(머니투데이 `&#039;` 등)이 실제로 티커에 깨진 문자 없이 잘 나오는지 admin/티커에서 한 번 육안 확인하면 좋음
...
# 다른 터미널에서:
curl "http://localhost:8788/__scheduled?cron=*/5+*+*+*+*"   # kr
curl "http://localhost:8788/__scheduled?cron=1-59/5+*+*+*+*" # us
curl "http://localhost:8788/__scheduled?cron=2-59/5+*+*+*+*" # en
curl "http://localhost:8788/__scheduled?cron=3-59/5+*+*+*+*" # jp
